...
import Countrycompare from './Countrycompare.js';
...
export default function Compare(props){
    ...
    const saveCountry = (value) => {
        setCountry(value)
        let data = value.map(x => {return x.country})
        setCountryList(data)
    }
    return(
        <div className="graphical-view">
            ...
            {country.length && open === false ? <button onClick={showselectcountry} className="btn btn-smm btn-outline d-flex align-items-center contentCenter mt-3 justify-content-end mr-3" type="button">
            ...
            {country.length === 0 && open === false ?
                ...
                open === true ? null : <Countrycompare list={list} update_country={country}></Countrycompare>
            }    
